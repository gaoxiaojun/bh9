...
/**
     * @interface EWrapper
     * @brief This interface's methods are used by the TWS/Gateway to
 * communicate with the API client.
     * Every API client application needs to implement this interface in order
 * to handle all the events generated by the TWS/Gateway. Almost every
...
public:
  ...
  /**
   * @brief Server's current time. This method will receive IB server's system
   ...
   * @param canAutoExecute Specifies whether the price tick is available for
   * automatic execution (1) or not (0).
   * @sa TickType, tickSize, tickString, tickEFP, tickGeneric,
   ...
   * @sa TickType, tickPrice, tickString, tickEFP, tickGeneric,
   * tickOptionComputation, tickSnapshotEnd, marketDataType,
   * EClientSocket::reqMktData
   */
   ...
   * @param value
   * @sa TickType, tickSize, tickPrice, tickEFP, tickGeneric,
   * tickOptionComputation, tickSnapshotEnd, marketDataType,
   * EClientSocket::reqMktData
   */
   ...
   * @param basisPoints Annualized basis points, which is representative of the
   ...
   * @param dividendImpact The dividend impact upon the annualized basis points
   * interest rate.
  ...
  /**
   * @brief -
   * Upon accepting a Delta-Neutral DN RFQ(request for quote), the server sends
   * a deltaNeutralValidation() message with the
   * UnderComp structure. If the delta and price fields are empty in the
   * original request, the confirmation will contain the current
   * values from the server. These values are locked when RFQ is processed and
   * remain locked unitl the RFQ is cancelled.
   ...
   * @param tickerId the request's unique identifier.
   * @param field Specifies the type of option computation. Pass the field value
   * into TickType.getField(int tickType) to retrieve the field description. For
   * example, a field value of 13 will map to modelOptComp, etc.
   *      10 = Bid
   *      11 = Ask
   *      12 = Last
   * @param impliedVolatility the implied volatility calculated by the TWS
   * option modeler, using the specified tick type value.
   * @param delta the option delta value.
   * @param optPrice the option price.
   * @param pwDividend the present value of dividends expected on the option's
   * underlying.
   ...
   * @sa TickType, tickSize, tickPrice, tickEFP, tickGeneric, tickString,
  ...
  /**
   * @brief When requesting market data snapshots, this market will indicate the
   ...
   * This method will receive the account information just as it appears in the
   ...
   *      - NetLiquidation — The basis for determining the price of the assets
   * in your account. Total cash value + stock value + options value + bond
   * value
   *      - TotalCashValue — Total cash balance recognized at the time of trade
   * + futures PNL
   *      - SettledCash — Cash recognized at the time of settlement - purchases
   * at the time of trade - commissions - taxes - fees
   ...
   * value of securities that one may purchase in a securities account without
   * depositing additional funds
   *      - EquityWithLoanValue — Forms the basis for determining whether a
   * client has the necessary assets to either initiate or maintain security
   * positions. Cash + stocks + bonds + mutual funds
   *      - PreviousEquityWithLoanValue — Marginable Equity with Loan value as
   * of 16:00 ET the previous day
   *      - GrossPositionValue — The sum of the absolute value of all stock and
   * equity option positions
   *      - RegTEquity — Regulation T equity for universal account
   *      - RegTMargin — Regulation T margin for universal account
   *      - SMA — Special Memorandum Account: Line of credit created when the
   * market value of securities in a Regulation T account increase in value
   ...
   *      - FullMaintMarginReq — Maintenance Margin of whole portfolio with no
   * discounts or intraday credits
   ...
   *      - LookAheadMaintMarginReq — Maintenance Margin requirement of whole
   * portfolio as of next period's margin change
   ...
   * liquidation
   *      - DayTradesRemaining — The Number of Open/Close trades a user could
   * put on before Pattern Day Trading is detected. A value of "-1" means that
  ...
  /*
   * @brief Delivers the Bond contract data after this has been requested via
   * reqContractDetails
   * @param reqId the request's identifier
   ...
   *      - AccountOrGroup — "All" to return account summary data for all
   * accounts, or set to a specific Advisor Account Group name that has already
   * been created in TWS Global Configuration
   *      - AccountReady — For internal use only
   *      - AccountType — Identifies the IB account structure
   *      - AccruedCash — Total accrued cash value of stock, commodities and
   * securities
   *      - AccruedCash-C — Reflects the current's month accrued debit and
   * credit interest to date, updated daily in commodity segment
   *      - AccruedCash-S — Reflects the current's month accrued debit and
   * credit interest to date, updated daily in security segment
   *      - AccruedDividend — Total portfolio value of dividends accrued
   *      - AccruedDividend-C — Dividends accrued but not paid in commodity
   * segment
   *      - AccruedDividend-S — Dividends accrued but not paid in security
   * segment
   *      - AvailableFunds — This value tells what you have available for
   * trading
   ...
   *      - BuyingPower — Cash Account: Minimum (Equity with Loan Value,
   * Previous Day Equity with Loan Value)-Initial Margin, Standard Margin
   * Account: Minimum (Equity with Loan Value, Previous Day Equity with Loan
   * Value) - Initial Margin *4
   *      - CashBalance — Cash recognized at the time of trade + futures PNL
   *      - CorporateBondValue — Value of non-Government bonds such as corporate
   * bonds and municipal bonds
   *      - Currency — Open positions are grouped by currency
   *      - Cushion — Excess liquidity as a percentage of net liquidation value
   *      - DayTradesRemaining — Number of Open/Close trades one could do before
   * Pattern Day Trading is detected
   *      - DayTradesRemainingT+1 — Number of Open/Close trades one could do
   * tomorrow before Pattern Day Trading is detected
   *      - DayTradesRemainingT+2 — Number of Open/Close trades one could do two
   * days from today before Pattern Day Trading is detected
   *      - DayTradesRemainingT+3 — Number of Open/Close trades one could do
   * three days from today before Pattern Day Trading is detected
   *      - DayTradesRemainingT+4 — Number of Open/Close trades one could do
   * four days from today before Pattern Day Trading is detected
   *      - EquityWithLoanValue — Forms the basis for determining whether a
   * client has the necessary assets to either initiate or maintain security
   * positions
   *      - EquityWithLoanValue-C — Cash account: Total cash value + commodities
   * option value - futures maintenance margin requirement + minimum (0, futures
   * PNL) Margin account: Total cash value + commodities option value - futures
   * maintenance margin requirement
   *      - EquityWithLoanValue-S — Cash account: Settled Cash Margin Account:
   * Total cash value + stock value + bond value + (non-U.S. & Canada securities
   * options value)
   *      - ExcessLiquidity — This value shows your margin cushion, before
   * liquidation
   *      - ExcessLiquidity-C — Equity with Loan Value - Maintenance Margin
   *      - ExcessLiquidity-S — Net Liquidation Value - Maintenance Margin
   *      - ExchangeRate — The exchange rate of the currency to your base
   * currency
   *      - FullAvailableFunds — Available funds of whole portfolio with no
   * discounts or intraday credits
   ...
   *      - FullExcessLiquidity — Excess liquidity of whole portfolio with no
   * discounts or intraday credits
   *      - FullExcessLiquidity-C — Net Liquidation Value - Full Maintenance
   * Margin
   *      - FullExcessLiquidity-S — Equity with Loan Value - Full Maintenance
   * Margin
   *      - FullInitMarginReq — Initial Margin of whole portfolio with no
   * discounts or intraday credits
   *      - FullInitMarginReq-C — Initial Margin of commodity segment's
   * portfolio with no discounts or intraday credits
   *      - FullInitMarginReq-S — Initial Margin of security segment's portfolio
   * with no discounts or intraday credits
   *      - FullMaintMarginReq — Maintenance Margin of whole portfolio with no
   * discounts or intraday credits
   *      - FullMaintMarginReq-C — Maintenance Margin of commodity segment's
   * portfolio with no discounts or intraday credits
   *      - FullMaintMarginReq-S — Maintenance Margin of security segment's
   * portfolio with no discounts or intraday credits
   *      - FundValue — Value of funds value (money market funds + mutual funds)
   *      - FutureOptionValue — Real-time market-to-market value of futures
   * options
   *      - FuturesPNL — Real-time changes in futures value since last
   * settlement
   *      - FxCashBalance — Cash balance in related IB-UKL account
   *      - GrossPositionValue — Gross Position Value in securities segment
   *      - GrossPositionValue-S — Long Stock Value + Short Stock Value + Long
   * Option Value + Short Option Value
   *      - IndianStockHaircut — Margin rule for IB-IN accounts
   *      - InitMarginReq — Initial Margin requirement of whole portfolio
   *      - InitMarginReq-C — Initial Margin of the commodity segment in base
   * currency
   *      - InitMarginReq-S — Initial Margin of the security segment in base
   * currency
   ...
   *      - LookAheadAvailableFunds — This value reflects your available funds
   * at the next margin change
   *      - LookAheadAvailableFunds-C — Net Liquidation Value - look ahead
   * Initial Margin
   *      - LookAheadAvailableFunds-S — Equity with Loan Value - look ahead
   * Initial Margin
   *      - LookAheadExcessLiquidity — This value reflects your excess liquidity
   * at the next margin change
   *      - LookAheadExcessLiquidity-C — Net Liquidation Value - look ahead
   * Maintenance Margin
   *      - LookAheadExcessLiquidity-S — Equity with Loan Value - look ahead
   * Maintenance Margin
   *      - LookAheadInitMarginReq — Initial margin requirement of whole
   * portfolio as of next period's margin change
   *      - LookAheadInitMarginReq-C — Initial margin requirement as of next
   * period's margin change in the base currency of the account
   *      - LookAheadInitMarginReq-S — Initial margin requirement as of next
   * period's margin change in the base currency of the account
   *      - LookAheadMaintMarginReq — Maintenance margin requirement of whole
   * portfolio as of next period's margin change
   *      - LookAheadMaintMarginReq-C — Maintenance margin requirement as of
   * next period's margin change in the base currency of the account
   *      - LookAheadMaintMarginReq-S — Maintenance margin requirement as of
   * next period's margin change in the base currency of the account
   *      - MaintMarginReq — Maintenance Margin requirement of whole portfolio
   ...
   *      - MoneyMarketFundValue — Market value of money market funds excluding
   * mutual funds
   *      - MutualFundValue — Market value of mutual funds excluding money
   * market funds
   *      - NetDividend — The sum of the Dividend Payable/Receivable Values for
   * the securities and commodities segments of the account
   *      - NetLiquidation — The basis for determining the price of the assets
   * in your account
   *      - NetLiquidation-C — Total cash value + futures PNL + commodities
   * options value
   *      - NetLiquidation-S — Total cash value + stock value + securities
   ...
   *      - PostExpirationExcess — Total projected "at expiration" excess
   * liquidity
   *      - PostExpirationExcess-C — Provides a projected "at expiration" excess
   * liquidity based on the soon-to expire contracts in your portfolio in
   * commodity segment
   *      - PostExpirationExcess-S — Provides a projected "at expiration" excess
   * liquidity based on the soon-to expire contracts in your portfolio in
   * security segment
   *      - PostExpirationMargin — Total projected "at expiration" margin
   *      - PostExpirationMargin-C — Provides a projected "at expiration" margin
   * value based on the soon-to expire contracts in your portfolio in commodity
   * segment
   *      - PostExpirationMargin-S — Provides a projected "at expiration" margin
   * value based on the soon-to expire contracts in your portfolio in security
   * segment
   *      - PreviousDayEquityWithLoanValue — Marginable Equity with Loan value
   * as of 16:00 ET the previous day in securities segment
   *      - PreviousDayEquityWithLoanValue-S — IMarginable Equity with Loan
   * value as of 16:00 ET the previous day
   *      - RealCurrency — Open positions are grouped by currency
   *      - RealizedPnL — Shows your profit on closed positions, which is the
   * difference between your entry execution cost and exit execution costs, or
   * (execution price + commissions to open the positions) - (execution price +
   ...
   *      - SMA — Line of credit created when the market value of securities in
   * a Regulation T account increase in value
   *      - SMA-S — Regulation T Special Memorandum Account balance for security
   * segment
   ...
   *      - TotalCashValue — Total cash value of stock, commodities and
   * securities
   ...
   *      - UnrealizedPnL — The difference between the current market value of
   * your open positions and the average cost, or Value - Average Cost
   *      - WarrantValue — Value of warrants
   *      - WhatIfPMEnabled — To check projected margin requirements under
   * Portfolio Margin model
   * @param value up-to-date value
   * @param currency the currency on which the value is expressed.
   * @param accountName the account
   * @sa updatePortfolio, updateAccountTime, accountDownloadEnd,
   ...
   * This function will receive only the portfolio of the subscribed account. If
   * the portfolios of all managed accounts are needed, refer to
   * EClientSocket::reqPosition
   ...
   * @sa updateAccountTime, accountDownloadEnd, updateAccountValue,
   * EClientSocket::reqAccountUpdates
   */
   ...
   * @sa updatePortfolio, accountDownloadEnd, updateAccountValue,
   * EClientSocket::reqAccountUpdates
   */
   ...
   * @sa updateAccountTime, updatePortfolio, updateAccountValue,
   * EClientSocket::reqAccountUpdates
   */
   ...
   * have not yet received confirmation that it has been accepted by the order
   ...
   *      ApiCanceled - after an order has been submitted and before it has been
   * acknowledged, an API client client can request its cancelation, producing
   * this state.
   *      Cancelled - indicates that the balance of your order has been
   * confirmed canceled by the IB system. This could occur unexpectedly when IB
   ...
   * @param parentId parent's id. Used for bracker and auto trailing stop
   * orders.
   ...
   * trying to locate shares for a short sell. The value used to indicate this
   * is 'locate'.
   * @sa openOrder, openOrderEnd, EClientSocket::placeOrder,
   ...
   * @sa orderStatus, openOrderEnd, EClientSocket::placeOrder,
   * EClientSocket::reqAllOpenOrders, EClientSocket::reqAutoOpenOrders
   */
   ...
   * @sa orderStatus, openOrder, EClientSocket::placeOrder,
   * EClientSocket::reqAllOpenOrders, EClientSocket::reqAutoOpenOrders
   */
   ...
   * This method will return all contracts matching the requested via
   * EClientSocket::reqContractDetails. For example, one can obtain the whole
  ...
  /**
   * @brief After all contracts matching the request were returned, this method
   * will mark the end of their reception.
   * @param reqId the request's identifier
   ...
   * @sa execDetailsEnd, commissionReport, EClientSocket::reqExecutions,
   * Execution
   */
   ...
   * @sa execDetails, execDetailsEnd, EClientSocket::reqExecutions,
   * CommissionReport
   */
   ...
   * @param reqId the request's identifier
   * @param date the bar's date and time (either as a yyyymmss hh:mm:ss
   ...
   */
  //virtual void historicalDataEnd(int reqId, const std::string &start,
  //                               const std::string &end);

  /**
   * @brief Returns the current market data type (real time streamed, frozen,
   * delayed, delayed-frozen)
   * @param reqId the request's identifier
   * @param marketDataType 1 for real time, 2 for frozen, 3 for delayed, 4 for
   * delayed-frozen
  ...
  /**
   * @brief provides the xml-formatted parameters available to create a market
   * scanner.
   ...
   * @sa scannerParameters, scannerDataEnd,
   ...
   *    1. Groups: offer traders a way to create a group of accounts and apply a
   * single allocation method to all accounts in the group.
   *    2. Profiles: let you allocate shares on an account-by-account basis
  ...
  * @param It returns a list of integers representing visible Group ID separated
  * by the "|" character, and sorted by most used group first.
  * @sa EClientSocket::queryDisplayGroups
  */
  virtual void displayGroupList(int reqId, const std::string &groups) = 0;

  /**
  * @brief call triggered once after receiving the subscription request, and
  * will be sent again if the selected contract in the subscribed * display
  * group has changed.
  * @param reqId the ID of the request
  * @param contractInfo
  ...
  */
  ...
  /**
  * @brief returns the option chain for an underlying on an exchange specified
  * in reqSecDefOptParams
  * There will be multiple callbacks to securityDefinitionOptionParameter if
  * multiple exchanges are specified in reqSecDefOptParams
  ...
  * @param expirations a list of the expiries for the options of this underlying
  * on this exchange
  * @param strikes a list of the possible strikes for options of this underlying
  * on this exchange
  ...
  */
  ...
  /**
  * @brief called when all callbacks to securityDefinitionOptionParameter are
  * complete
...
errorString) = 0;
    // market data
   virtual void tickPrice( TickerId tickerId, TickType field, double price, int
canAutoExecute) = 0;
   virtual void tickSize( TickerId tickerId, TickType field, int size) = 0;
   virtual void tickOptionComputation( TickerId tickerId, TickType tickType,
double impliedVol, double delta,
           double optPrice, double pvDividend, double gamma, double vega, double
theta, double undPrice) = 0;
...
basisPoints, const std::string& formattedBasisPoints,
           double totalDividends, int holdDays, const std::string&
futureLastTradeDate, double dividendImpact, double dividendsToLastTradeDate) =
0;
    virtual void updateMktDepth(TickerId id, int position, int operation, int
side,
       double price, int size) = 0;
    virtual void updateMktDepthL2(TickerId id, int position, std::string
marketMaker, int operation,
       int side, double price, int size) = 0;
    virtual void updateNewsBulletin(int msgId, int msgType, const std::string&
newsMessage, const std::string& originExch) = 0;

    // order
    virtual void orderStatus( OrderId orderId, const std::string& status, double
filled,
           double remaining, double avgFillPrice, int permId, int parentId,
           double lastFillPrice, int clientId, const std::string& whyHeld) = 0;
   virtual void openOrder( OrderId orderId, const Contract&, const Order&, const
OrderState&) = 0;
   virtual void openOrderEnd() = 0;

    // account
   virtual void updateAccountValue(const std::string& key, const std::string&
val,
...
= 0;
   ...
   virtual void nextValidId( OrderId orderId) = 0;
    // contract
   virtual void contractDetails( int reqId, const ContractDetails&
contractDetails) = 0;
   virtual void bondContractDetails( int reqId, const ContractDetails&
contractDetails) = 0;
   virtual void contractDetailsEnd( int reqId) = 0;
    // execution
   virtual void execDetails( int reqId, const Contract& contract, const
Execution& execution) =0;
   virtual void execDetailsEnd( int reqId) =0;

    // historical
   virtual void historicalData(TickerId reqId, const std::string& date, double
open, double high,
           double low, double close, int volume, int barCount, double WAP, int
hasGaps) = 0;
...
double position, double avgCost) = 0;
   virtual void positionEnd() = 0;
   virtual void accountSummary( int reqId, const std::string& account, const
std::string& tag, const std::string& value, const std::string& curency) = 0;
   virtual void accountSummaryEnd( int reqId) = 0;
   virtual void verifyMessageAPI( const std::string& apiData) = 0;
   virtual void verifyCompleted( bool isSuccessful, const std::string&
errorText) = 0;
   virtual void displayGroupList( int reqId, const std::string& groups) = 0;
   virtual void displayGroupUpdated( int reqId, const std::string& contractInfo)
= 0;
   virtual void verifyAndAuthMessageAPI( const std::string& apiData, const
std::string& xyzChallange) = 0;
   virtual void verifyAndAuthCompleted( bool isSuccessful, const std::string&
errorText) = 0;

   virtual void positionMulti( int reqId, const std::string& account,const
std::string& modelCode, const Contract& contract, double pos, double avgCost) =
0;
   virtual void positionMultiEnd( int reqId) = 0;
   virtual void accountUpdateMulti( int reqId, const std::string& account, const
std::string& modelCode, const std::string& key, const std::string& value, const
std::string& currency) = 0;
   virtual void accountUpdateMultiEnd( int reqId) = 0;
...
std::set<double> strikes) = 0;
   virtual void securityDefinitionOptionalParameterEnd(int reqId) = 0;
   virtual void softDollarTiers(int reqId, const std::vector<SoftDollarTier>
&tiers) = 0;
};*/
