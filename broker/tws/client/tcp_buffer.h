...
/**
 * @brief The tcp_buffer class
 * @note  Usage:
 *         for reader:  async_read_some(prepare(), bind(...));
 *                      retrieve(bytes_transferred);
 *                      const char *begin = peek();
 *                      const char *end   = begin + size();
 *                      consume(actual_processed);
 *         for writter: append(const void *data, size_t len);
 *                      async_write_some(data(), bind(...));
